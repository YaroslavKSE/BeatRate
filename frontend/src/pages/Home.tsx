...
import FollowingFeedComponent from '../components/Home/FollowingFeedComponent';
...
const Home = () => {
  ...
  return (
      <div className="max-w-6xl mx-auto py-2 sm:py-8 px-3 sm:px-4">
        {/* Hero Section with Search */}
        <div className="text-center mb-8 sm:mb-12 px-2 sm:px-4">
          <h1 className="text-xl sm:text-4xl font-bold text-gray-900 mb-2 sm:mb-3">Welcome to BeatRate</h1>
          <p className="text-sm sm:text-xl text-gray-600 mb-6 sm:mb-8">Discover, rate, and share your music
            experiences</p>
            ...
            <form onSubmit={handleSearchSubmit} className="relative">
              <div className="flex items-center relative">
                ...
                <input
                    ...
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="sm:hidden w-full py-2 px-3 pl-8 rounded-full text-sm focus:outline-none border border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 shadow-sm"
                    aria-label="Search for music"
                />
                <button
                    type="submit"
        ...
        <div
            className="flex overflow-x-auto space-x-3 sm:grid sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-12"
            ref={featureScrollRef}
        >
          {[
            {
              title: 'Rate Your Way',
              icon: <Star className="h-3 sm:h-5 w-3 sm:w-5 mr-2 text-primary-500"/>,
            ...
            {
              title: 'Share Reviews',
              icon: <Music className="h-3 sm:h-5 w-3 sm:w-5 mr-2 text-primary-500"/>,
            ...
            },
            {
              ...
              icon: <ListMusic className="h-3 sm:h-5 w-3 sm:w-5 mr-2 text-primary-500"/>,
              description: 'Create lists of your favorite tracks and albums.',
              link: '/lists',
              linkText: 'Explore lists',
            },
          ].map((feature, index) => (
              <div key={index} className="bg-white shadow rounded-lg overflow-hidden min-w-[75%] sm:min-w-0">
                <div className="p-3 sm:p-6">
                  <h3 className="text-base sm:text-xl font-bold text-gray-900 mb-1 sm:mb-3 flex items-center">
                    {feature.icon}
                    {feature.title}
                  </h3>
                  <p className="text-gray-600 mb-2 sm:mb-4 text-xs sm:text-base">{feature.description}</p>
                  <Link
                      to={feature.link}
                      className="text-primary-600 hover:text-primary-800 font-medium flex items-center text-xs sm:text-base"
                  >
        ...
        {/* Following Feed section - only visible when authenticated */}
        {isAuthenticated && (
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden mb-8">
          <div
              className="px-3 py-3 sm:px-6 sm:py-4 bg-gradient-to-r from-primary-600 to-primary-500 flex justify-between items-center">
          ...
          {loading ? (
              ...
              <div className="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 p-4 sm:p-6"
                   role="list">
